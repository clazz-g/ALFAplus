...
hippo_data_long %>%
  ...
  mutate(A = factor(A, labels = c("A-", "A+"))) %>%
...
models<-list(
  mod_ca1 = lmer(vol_CA1 ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_ca3 = lmer(vol_CA3 ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_ca4 = lmer(vol_CA4 ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_gc = lmer(vol_GC_ML_DG ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_hata = lmer(vol_HATA ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_tail = lmer(vol_Hippocampal_tail ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_fimb = lmer(vol_fimbria ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_ml = lmer(vol_molecular_layer_HP ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_para = lmer(vol_parasubiculum ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_pre = lmer(vol_presubiculum ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot),
  mod_sub = lmer(vol_subiculum ~ kids_total*A + scale(TIV) + age_mri + APOE_binary + (1 | subject),data = data_forest_plot)
)
...
pred_hippo_all %>%
  ggplot(aes(x = estimate, y = region, group = A, color = A)) +
...
data %>% 
  ...
  summarise(
    n = n(),
    mean_diff = mean(diff_mri, na.rm = TRUE),
    sd_diff = sd(diff_mri, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  ggplot(aes(x = factor(sex), y = mean_diff)) +
  geom_col(fill = "steelblue", alpha = 0.7) +
  geom_errorbar(aes(ymin = mean_diff - sd_diff, ymax = mean_diff + sd_diff), 
                width = 0.2) +
  labs(x = "Sex", y = "Mean MRI time difference (years)") +
  th
