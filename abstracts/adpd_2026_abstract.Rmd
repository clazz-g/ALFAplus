...
fit_cog<-lmer(Z_FS_totalcnswr ~ kids_total*A +age_cog + APOE_binary + education + Fuma2 + repro_span +(1 | subject), data_cog_long[data_cog_long$sex == 2,])
summary(fit_cog)
...
fit_cog_domain<-lmer(Z_Visual_Composite ~ kids_total*A +age_cog + APOE_binary + education +age_menopause+(1 | subject), data_cog_long_composites[data_cog_long_composites$sex == 2,])
...
cols<-data_cog_long_composites%>%
  dplyr::select(subject, Visit, Z_Attention_Composite, Z_Executive_Composite, Z_Memory_Composite, Z_Language_Composite, Z_Visual_Composite)
...
pred_hippo_all %>%
  ...
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), 
                  ...
                  size = 0.6) +
  labs(x = "Estimated effect of parity on volume change", 
