...
fit_cog_domain<-lmer(Z_Visual_Composite ~ kids_total*A +age_cog + APOE_binary + education + (1 | subject), data_cog_long_composites[data_cog_long_composites$sex == 2,])
...
fit_cog_domain_long<-lmer(Z_Attention_Composite ~ kids_total*A*time_diff +age_v1 + APOE_binary + education + (1 | subject), data_cog_long_composites[data_cog_long_composites$sex == 2,])
summary(fit_cog_domain)
...
diff_biomarkers<-biomarkers_long%>%
  dplyr::select(subject, Visit, collection_dt)%>%
  pivot_wider(names_from = Visit, values_from = collection_dt)%>%
  mutate(V1 = as.Date (V1, format= "%d/%m/%Y"),
         V2 = as.Date (V2, format= "%d/%m/%Y"),
         diff_biomarkers = as.numeric(difftime(V2, V1, units= c("days"))))%>%
  dplyr::select(diff_biomarkers, subject)

data_biomarkers_long<-left_join(data_naked, biomarkers_long)%>%
  left_join(., diff_biomarkers)%>%
  mutate(diff_biomarkers = ifelse(Visit == "V1", 0, diff_biomarkers))

fit_nfl <- lmer(CSF_NFL ~ kids_total*diff_biomarkers  +age_v1 + APOE_binary + (1 | subject) , data_biomarkers_long[data_biomarkers_long$sex==2,]) 
summary(fit_nfl)
...
#Mediation test
does not yield much
should I test mediation with biomarker?
```{r}
