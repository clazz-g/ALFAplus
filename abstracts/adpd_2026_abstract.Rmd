...
emm_at_times <- emmeans(fit_cog, ~ kids_total * A * time_diff, 
                       at = list(kids_total = c(0, 1, 2, 3, 4),
                                 time_diff = c(0, 1000, 1500, 2000)))

emm_at_times
pairs(emm_3way, simple = "kids_total", combine = TRUE)

slopes <- emtrends(fit_cog, ~ kids_total * A, var = "time_diff",
                  at = list(kids_total = c(0, 1, 2, 3, 4)))
pairs(slopes, simple = "kids_total")

emmip(fit_cog, A ~ time_diff | kids_total, 
      at = list(kids_total = c(0, 1, 2, 3, 4),
               time_diff = seq(0, 1000, 1500)),  # Adjust range
      CIs = TRUE)
...
hist(data_cog_long_composites$time_diff)
```
...
data_figure_cog%>%
  ...
  mutate(kids_total_factor=ifelse(kids_total>=3, "3+", kids_total))%>%
  mutate(A = factor(A, levels = c("0", "1"), labels = c("Aβ-", "Aβ+")))%>%
  ggplot(aes(x= time_diff, y = diff_Z_PACC, color= kids_total_factor, fill= kids_total_factor, group=subject))+
  geom_line(alpha=.6)+
  facet_wrap(~A)+
  geom_smooth(aes(group = kids_total_factor), method="lm")
...
Results: Parity did not show a significant positive main effect on volume in the fimbria. A significant parity * Aβ status interaction was found in the parasubiculum (adj.p.=0.020), where Aβ+ women with higher parity showed smaller volumes (Figure 1 and 2). This interaction was trending in the CA4 (adj.p=0.055), dentate gyrus (adj.p=0.062), presubiculum (adj.p=0.051) and subiculum (adj.p=0.058). A significant parity * Aβ status * time was found in PACC score (p=0.033), where Aβ+ women with higher parity showed worsening score over time. 

Conclusion: In CU post-menopausal women at risk of AD, parity interacts with AD pathology to influence hippocampus subfield volumes across visits, in particular the parasubiculum. Parity and AD pathology also impact cognition longitudinally. These findings suggest that reproductive history may interact with AD neuro and cognitive pathology. 
