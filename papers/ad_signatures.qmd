...
We foudn no significant independent effect of parity on whole hippocampal volume. However, a trending parity and Aβ positivity interaction was found (β = `r tidy(mod_whole_int)$estimate[10]`, p = `r tidy(mod_whole_int)$p.value[10]`), such that Aβ- women showed a positive relationship between parity and hippocampal volume, whilst Aβ- women showed the opposite (Figure X).
...
mod_whole_intA0_CSF<-lmer(vol_Whole_hippocampus ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education + CSF_NFL + (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_whole_intA0_CSF)


#synaptotagmin trend, GAP43 trend, sTREM2 very significant, 
```

No significant interaction with time was found, indicating that this effect appears constant over visits. Subsequent stratification of the analysis revealed a significant main effect of parity on hippocampus volume only in Aβ- women (β = `r tidy(mod_whole_intA0)$estimate[2]` , p=`r tidy(mod_whole_intA0)$p.value[2]`), but no interaction with time.

No significant main effect of parity or interaction between parity and Aβ positivity was found in men.
...
mod_para_A0<-lmer(vol_parasubiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + education+ scale(TIV)  +(1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_para_A0)

mod_para_A1<-lmer(vol_parasubiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + education+ scale(TIV) + (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==1,])
summary(mod_para_A1)


#parasubiculum driven by A+ women

mod_CA4_A0<-lmer(vol_CA4 ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_CA4_A0)
mod_CA4_A1<-lmer(vol_CA4 ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==1,])
summary(mod_CA4_A1)

#CA4 driven by A- women. sTREM2 very significant

mod_DG_A0<-lmer(vol_GC_ML_DG ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_DG_A0)
mod_DG_A1<-lmer(vol_GC_ML_DG ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==1,])
summary(mod_DG_A1)

#DG driven by A- women sTREM2 highly significant

mod_subiculum_A0<-lmer(vol_subiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_subiculum_A0)
mod_subiculum_A1<-lmer(vol_subiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education+ (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==1,])
summary(mod_CA4_A1)

#subiculum driven by A- women, sTREM2 sig

mod_presubiculum_A0<-lmer(vol_presubiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education + (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==0,])
summary(mod_presubiculum_A0)
mod_presubiculum_A1<-lmer(vol_presubiculum ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education + (1|subject), data = hippo_data_long[hippo_data_long$sex==2&hippo_data_long$A==1,])
summary(mod_presubiculum_A1)

#presubiculum driven by A- women, sTREM2 sig

mod_ML_A0<-lmer(vol_molecular_layer_HP ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education + (1|subject), data = hippo_data_long[hippo_data_long$sex==1&hippo_data_long$A==0,])
summary(mod_ML_A0)
mod_ML_A1<-lmer(vol_molecular_layer_HP ~ kids_total*diff_mri + age_v1 +APOE_binary + scale(TIV) + education + (1|subject), data = hippo_data_long[hippo_data_long$sex==1&hippo_data_long$A==1,])
summary(mod_ML_A1)
```

Susequent stratification of the regressions by Aβ status showed that the effect was driven by Aβ- women in all subfields, but the parasubiculum, where the effect seemed to be driven by Aβ+ women.

In men, no significant main effect of parity was found on any of the hippocampal subfields. However, a significant three-way interaction was found between parity, time and Aβ status in the molecular layer (β = `r tidy_models_hip_w_int_men$estimate[8]`, adj.p = `r fdr_hip_w_int_men[8]`).

Stratification by Aβ status shows the effect is driven by Aβ+ men. 
...
mod_ashs<-lmer(vol_CA1 ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_CA2 ~ kids_total*diff_mri*A + age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_CA3 ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_DG ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_SUB ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_ERC ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_PHC ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_BA35 ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)

mod_ashs<-lmer(vol_BA36 ~  kids_total*diff_mri*A +age_v1 +APOE_binary + scale(vol_tiv) + education +(1|subject), data = data_ashs[data_ashs$sex==2,])
summary(mod_ashs)
