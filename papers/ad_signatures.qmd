...
g <- dagitty("dag{  
  ...
  age_v1 -> {kids_total hypertension Hormones HRT_pmm vol_Whole_hippocampus A}
  kids_total -> {hypertension Hormones}
  Hormones -> {repro_span hypertension vol_Whole_hippocampus};
  education -> {kids_total hypertension ingreso_hogar vol_Whole_hippocampus A};
  repro_span -> {HRT_pmm hypertension vol_Whole_hippocampus};
  HRT_pmm -> hypertension ;
  ingreso_hogar -> {hypertension};
  hypertension -> {A vol_Whole_hippocampus};
...
data_dag_v1<-hippo_data_long%>%
  ...
  mutate(APOE_binary= as.integer(APOE_binary),
         A=as.integer(A))%>%
  dplyr::select(subject, kids_total, repro_span, education, HRT_pmm, hypertension, high_chol, diabetes, vol_Whole_hippocampus, ingreso_hogar, A)
...
plot(data_dag_v1$kids_total, data_dag_v1$ingreso_hogar)
```
