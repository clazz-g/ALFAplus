...
Our sample includes `r pt_number` participants from the ALFA+ project, of which `r pt_number_w` are women. In addition to the above exclusion criteria, women with history of cardiovascular related gestational disease, such as preeclampsia, were excluded from our sample for analysis. Participants were recruited via advertisement campaigns in newspapers and followed-up via email/phonecalls.
...
g <- dagitty("dag{ 
  Parity -> CVR;
  Parity -> Hormones;
  Hormones -> Reproductive_span;
  Hormones -> CVR;
  Education -> Parity ;
  Reproductive_span -> HRT;
  HRT -> CVR ;
  HRT -> PACC;
  Reproductive_span-> CVR ;
  Reproductive_span-> PACC;
  Reproductive_span-> Hippocampal_volume;
  Education -> CVR ;
  Education -> Income;
  Education -> Hippocampal_volume;
  Income -> CVR;
  CVR -> A;
  CVR -> Hippocampal_volume;
  CVR -> PACC
  A -> PACC
 }")
...
data_bn<-data_sem%>%
  filter(sex==2)%>%
  dplyr::select(kids_total, HRT, age_v1, repro_span, hypertension, high_chol, CSF_Neurogranin_V1, CSF_Neurogranin_V2, CSF_GFAP_V1, CSF_GFAP_V2, diabetes, `CSF_IL-6_V1`, `CSF_IL-6_V2`, vol_Whole_hippocampus_V1 , vol_Whole_hippocampus_V2, CSF_A_V1, CSF_A_V2, CSF_sTREM2_V1, CSF_sTREM2_V2, Z_PACC_V1, Z_PACC_V2, bmi, occupation, ingreso_hogar, education)%>%
...
bl = tiers2blacklist(list("HRT", "age_v1", "repro_span", "occupation", "education",
       c("hypertension", "high_chol", "CSF_Neurogranin_V1", "CSF_GFAP_V1", "diabetes", "CSF_IL_6_V1", "vol_Whole_hippocampus_V1", "CSF_A_V1", "Z_PACC_V1", "CSF_sTREM2_V1", "ingreso_hogar", "bmi")))
bl = rbind(bl, c("kids_total", "repro_span"), c("repro_span", "kids_total"))
bl = rbind(bl, c("kids_total", "age_v1"), c("age_v1", "kids_total"))
bl = rbind(bl, c("repro_span", "HRT"), c("HRT", "repro_span"))


#whitelist
wl = matrix(c(
              "repro_span", "hypertension",
             "CSF_A_V1", "vol_Whole_hippocampus_V1",
             "vol_Whole_hippocampus_V1", "Z_PACC_V1",
             "CSF_A_V1", "Z_PACC_V1",
             "education", "Z_PACC_V1",
...
dag = hc(data_bn, blacklist = bl, whitelist = wl)
dag

graphviz.plot(dag, shape = "ellipse")


str.diff = boot.strength(data_bn, R = 200, algorithm = "hc", algorithm.args = list(whitelist = wl, blacklist = bl))
...
fitted.simpler = bn.fit(avg.simpler, data_bn)
fitted.simpler
...
const = "kids_total"
t1.variables = grep("V1$", names(data_bn), value = TRUE)
t1.variables
t2.variables = grep("V2$", names(data_bn), value = TRUE)
t2.variables
...
roots = expand.grid(from = setdiff(names(data_bn), c("HRT", "age_v1", "repro_span", "occupation", "education")), to = c("HRT", "age_v1", "repro_span", "occupation", "education"), stringsAsFactors = FALSE)
...
dyn.dag = tabu(data_bn, blacklist = bl, whitelist = wl)
dyn.dag
...
dyn.str = boot.strength(data_bn, R = 200, algorithm = "tabu",
...
med.fit<-lme4::lmer(vol_parasubiculum ~ kids_total*diff_mri + scale(age_v1)  + scale(TIV) + scale(education) + (1|subject), data = data_med_A0[data_med_A0$sex==2 ,])

summary(med.fit)
out.fit<-lme4::lmer(Z_PACC ~ vol_parasubiculum + kids_total*diff_mri + scale(age_v1)  + scale(TIV) + scale(education) + (1|subject) , data = data_med_A0[data_med_A0$sex==2 ,])
summary(out.fit)

med.out <- mediate(med.fit, out.fit, treat = "kids_total", mediator = "vol_parasubiculum", sims = 100)
summary(med.out)
...
Limitations: - unequal number of men and women - still early stages - uniformity of sample both ethnically and socially - no more than 4 kids per women
