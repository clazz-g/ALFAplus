...
data%>%
  filter(sex==1)%>%
  dplyr::select(diff_PACC)%>%
  hist()

mod<-lm(Z_PACC_v1 ~ kids_total*A + age_v1 + education + APOE_binary +age_menopause_pmm, data = data[data$sex ==2,])
summary(mod)
...
#dbatable significance
anna_data<-read_excel("C:/Users/U253662/Downloads/ALFA+_dataset_pregnancy_14052025onlywomen.xlsx")%>%
  mutate(A = ifelse(ab42_40_ratio>=0.071, 0, 1),
         ID = as.numeric(ID))%>%
  rename(subject = ID)
