...
mice_dat<-master_nd%>%
  filter(sex == 2)%>%
  dplyr::select(age_menarche, subject, age_v1, sex, BP_SIST_2, bmi, education , APOE_binary)%>%
  distinct()
...
age_menarche_pmm<-data_imp_multi_all%>%
  dplyr::select(2:7)%>%
  mutate(age_menarche_pmm = rowMeans(across(starts_with("age_menarche."))))%>%
  rename(subject=subject.0)%>%
  mutate(subject = factor(subject))


master_nd_pmm<-left_join(master_nd_pmm, age_menarche_pmm[,c(6, 7)])

#hrt

mice_dat<-master_nd%>%
  filter(sex == 2)%>%
  dplyr::select(HRT, subject, age_v1, sex, BP_SIST_2, bmi, education , APOE_binary)%>%
  distinct()

ini <- mice(mice_dat,maxit=0)
pred1 <- ini$predictorMatrix
pred1[,'subject'] <- 0
imp_multi <- mice(mice_dat, pred = pred1, m = 5, method = "logreg")  # Impute missing values multiple times
data_imp_multi_all <- complete(imp_multi,       # Store multiply imputed data
                           "repeated",
                           include = TRUE)

HRT_pmm<-data_imp_multi_all%>%
  dplyr::select(2:7)%>%
  rowwise() %>%
  mutate(HRT_pmm = {
    values <- c_across(starts_with("HRT."))
    as.numeric(names(sort(table(values), decreasing = TRUE))[1])
  }) %>%
  ungroup() %>%
  # Add subject back
  mutate(subject = data_imp_multi_all$subject.0) %>%
  mutate(subject = factor(subject)) %>%
  dplyr::select(subject, HRT_pmm)



master_nd_pmm<-left_join(master_nd_pmm, HRT_pmm)
...
master_nd_pmm<-master_nd_pmm%>%
  mutate(repro_span = age_menopause_pmm - age_menarche_pmm,
