...
menopause<-read_delim("G:/My Drive/Data/ALFA+/data_raw/reproductive_var/Alfa Plus Long 2025_1054_AF_20250304095809(in).csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE, skip = 1)%>%
  dplyr::select(IdParticipante, F06_2_017, F06_2_019, F06_2_018, F06_2_015, Visita)%>%
  rename(subject = IdParticipante,
  ...
  filter(!if_all(c(age_menopause, abortions, preg_termino, age_menarche), is.na))%>%
  group_by(subject)%>%
  mutate(age_menopause = if (all(is.na(age_menopause))) NA else max(age_menopause, na.rm = TRUE),
         abortions = if (all(is.na(abortions))) NA else max(abortions, na.rm = TRUE),
         age_menarche = if (all(is.na(age_menarche))) NA else max(age_menarche, na.rm = TRUE))%>%
  ungroup()%>%
  unique()%>%
  mutate(subject = factor(subject),
...
master_nd<-left_join(master_nd, menopause)%>%
  mutate(is_meno = ifelse(age_v1>age_menopause, 0, 1))

#hard coded values retrieved from pt info
master_nd$is_meno[master_nd$subject == 10627]<-1
master_nd$age_menopause[master_nd$subject == 10627]<-54
master_nd$is_meno[master_nd$subject == 11099]<-1
master_nd$is_meno[master_nd$subject == 12125]<-1
master_nd$age_menopause[master_nd$subject == 12125]<-48
master_nd$is_meno[master_nd$subject == 12774]<-0



women_delivery_pat<-left_join(women_delivery_pat, menopause)
...
master_nd<-master_nd%>%
  mutate(age_menopause = ifelse(age_menopause == -77, NA, age_menopause),
         ...
         is_meno_v1 = ifelse(age_menopause>age_v1+0.5, 0, 1))

```

# New repro var
```{r}
# repro_var<-read_delim("G:/My Drive/Data/ALFA+/data_raw/reproductive_var/27_4_2026/A+ Long 2025 4Q_1054_AF_20251204105351.csv", skip=1)%>%
#   dplyr::select(IdParticipante, F06_2_017, F06_2_019, F06_2_018, F06_2_015, Visita)%>%
#   rename(subject = IdParticipante,
#          age_menopause = F06_2_017, 
#          abortions = F06_2_019,
...
# 
# menopause<-repro_var%>%
#   dplyr::select(subject, age_menopause)%>%
#   mutate(subject = as.numeric(as.character(subject)))
# 
# symptoms<-read_delim("G:/My Drive/Data/ALFA+/data_raw/reproductive_var/27_4_2026/A+ Long 2025 4Q_1054_AF_CRF_AF_SymptomsMenopause_20251204105351.csv", skip=1)%>%
#   dplyr::select(IdParticipante, Visita, AF_SympMnps_id, AF_SympMnps, AF_OnsetAgeSympMnps, AF_EndAgeSympMnps, AF_CmmSympMnps)%>%
#   rename(subject = IdParticipante, 
#          Visit = Visita)%>%
#   left_join(.,menopause)
#   
...
women_delivery_pat<-left_join(women_delivery_pat, apoe)
```
...
#write.csv(women_delivery_pat, file = "G:/My Drive/Data/ALFA+/data_cleaned/excluded_repro_var.csv")


biomarkers<-read_delim(paste0("G:/My Drive/Data/ALFA+/data_cleaned/biomarkers_CSF_processed.csv"), 
    delim = ",", escape_double = FALSE, trim_ws = TRUE)%>%
  mutate(A=factor(A))%>%
  filter(Visit == "V1")%>%
...
master_bio<-left_join(master_nd, biomarkers)
#write.csv(master_bio, file = "G:/My Drive/Data/ALFA+/data_cleaned/main_dataset_with_A.csv", row.names = F)
