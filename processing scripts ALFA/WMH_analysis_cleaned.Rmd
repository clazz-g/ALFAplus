...
```{r}
wmh_V1<- read_excel("G:/My Drive/Data/ALFA+/data_raw/WMH/bx_bamos_stats_ALFA_PLUS_V1_20231120_20231223_015120.xlsx")%>%
...
wmh_V2<- read_excel("G:/My Drive/Data/ALFA+/data_raw/WMH/bx_bamos_stats_ALFA_PLUS_V2_20230518_20231025_084845.xlsx")%>%
  dplyr::select(subject, volume, region, depth)%>%
  filter(region!= "background", 
         region!= "infratentorial")%>%
  mutate(region = factor(region))%>%
...
write.csv(wmh_data, file = "G:/My Drive/Data/ALFA+/data_cleaned/wmh_processed_regional_lobes.csv", row.names = F)
